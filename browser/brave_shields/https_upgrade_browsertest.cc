 ...
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    mock_cert_verifier_.mock_cert_verifier()->AddResultForCertAndHost(
        cert, "insecure1.test", verify_result,
        net::ERR_CERT_INVALID);
    mock_cert_verifier_.mock_cert_verifier()->AddResultForCertAndHost(
        cert, "insecure2.test", verify_result,
        net::ERR_CERT_INVALID);
    mock_cert_verifier_.mock_cert_verifier()->AddResultForCertAndHost(
        cert, "insecure3.test", verify_result,
        net::ERR_CERT_INVALID);
  ...
  void TearDownOnMainThread() override { SetHttpsOnlyPref(false); }
...
protected:
  void SetHttpsOnlyPref(bool enabled) {
    auto* prefs = browser()->profile()->GetPrefs();
    prefs->SetBoolean(prefs::kHttpsOnlyModeEnabled, enabled);
  }

  bool GetHttpsOnlyPref() const {
    auto* prefs = browser()->profile()->GetPrefs();
...
const TestStruct test_combinations[] = {
  ...
  { false, "upgradable2.test", ControlType::BLOCK_THIRD_PARTY, PageResult::HTTPS},
...
IN_PROC_BROWSER_TEST_F(HttpsUpgradeBrowserTest,
                       CheckUpgrades) {
  for (bool global_setting : { true, false }) {
    for (const TestStruct test : test_combinations) {
      GURL initial_url = test.init_secure ?
        https_server()->GetURL(test.domain, "/simple.html") :
        http_server()->GetURL(test.domain, "/simple.html");
      brave_shields::SetHttpsUpgradeControlType(ContentSettings(),
                                                test.control_type, global_setting ? GURL() : initial_url,
      ...
      bool interstitial_showing = chrome_browser_interstitials::IsShowingInterstitial(Contents());
