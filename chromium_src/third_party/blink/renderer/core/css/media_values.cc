...
#define CalculateDeviceWidth                       \
  CalculateDeviceWidth(LocalFrame* frame) {        \
    return frame->DomWindow()->screen()->width();  \
  }                                                \
  int MediaValues::CalculateDeviceWidthOriginal

#define CalculateDeviceHeight                      \
  CalculateDeviceHeight(LocalFrame* frame) {       \
    return frame->DomWindow()->screen()->height(); \
  }                                                \
  int MediaValues::CalculateDeviceHeightOriginal
...
#endif  // BRAVE_CHROMIUM_SRC_THIRD_PARTY_BLINK_RENDERER_CORE_CSS_MEDIA_VALUES_CC_
