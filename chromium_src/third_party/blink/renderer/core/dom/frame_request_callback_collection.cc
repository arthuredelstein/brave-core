...
void FrameRequestCallbackCollection::ExecuteFrameCallbacks(
  ...
  if (!brave::AllowTimeStampFingerprinting(context_)) {
    high_res_now_ms = brave::RoundTimeStamp(high_res_now_ms);
    high_res_now_ms_legacy = brave::RoundTimeStamp(high_res_now_ms_legacy);
  }
