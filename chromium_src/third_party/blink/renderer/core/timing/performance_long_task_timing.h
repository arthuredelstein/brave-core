...
#define entryType                                                        \
  *dummy() { return nullptr; }                                           \
  PerformanceLongTaskTiming(                                             \
      double start_time, int duration, const AtomicString& name,         \
      const AtomicString& culprit_type, const AtomicString& culprit_src, \
      const AtomicString& culprit_id, const AtomicString& culprit_name,  \
      const uint32_t navigation_id, bool allow_fingerprinting);          \
