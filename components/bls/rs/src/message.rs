...
#[serde(remote = "fvm_shared::message::Message")]
...
pub mod MessageAPI {
  ...
  pub fn serialize<S>(token_amount: &Message, serializer: S) -> Result<S::Ok, S::Error>
  where
      S: Serializer,
  {
      serializer.serialize_str(&s)
  ...
  {
      let s = String::deserialize(deserializer)?;
      Message::from_str(&s).map_err(de::Error::custom)
  }
}
*/
